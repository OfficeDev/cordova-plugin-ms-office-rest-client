...
(function (O365Auth) {
    ...
    var Context = (function () {
        ...
        Context.prototype.getAccessToken = function (resourceId, loginHint, clientId, redirectUri) {
            ...
            var authorizationUri = this._authUri +
                'oauth2/authorize?response_type=code&resource=' + encodeURIComponent(resourceId) +
                '&client_id=' + encodeURIComponent(clientId) +
                '&redirect_uri=' + encodeURIComponent(redirectUri) +
                (loginHint ? '&login_hint=' + encodeURIComponent(loginHint) : '');

            var handleRedirectUri = function (uri) {

                var code = this.getParameterByName(uri, 'code'),
                    error = this.getParameterByName(uri, 'error_description');

                if (code) {
                    var url = this._authUri + 'oauth2/token',
            ...
            if (!isWebAuthBrokerAvailable) {
                ...
                var onExit = function () {
                    deferred.reject(new Microsoft.Utility.Exception('The logon dialog was closed by user'));
                };

                var onRedirect = function (e) {
                    var url = e.url;

                    if (url.substr(0, redirectUri.length).toLowerCase() === redirectUri.toLowerCase()) {
                ...
                var successCallback = function (result) {
                    switch (result.responseStatus) {
                        ...
                        case webAuthStatus.userCancel:
                            ...
                            break;
                        default:
                            deferred.reject(new Microsoft.Utility.Exception(result));
                    }
                };
                ...
                if (webAuthBroker.authenticateAndContinue) {
                    ...
                    var callback = function (activationArgs) {
                        if (activationArgs.detail.kind === webAuthContinuation) {
                            ...
                            WinJS.Application.removeEventListener('activated', callback);
                        }
                    };
